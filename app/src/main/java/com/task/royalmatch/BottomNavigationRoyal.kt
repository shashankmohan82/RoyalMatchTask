...
public class BottomNavigationRoyal : BottomNavigationView {
    ...
    private var disabledColor =
        ContextCompat.getColor(context, com.google.android.material.R.color.material_on_surface_emphasis_medium)
    private var enabledColor = Color.WHITE
    private var textColor by Delegates.notNull<Int>()

    var backgroundShape = Shape.Circle
    private var customBackgroundDrawable = -1
    public var circleColor = Color.GREEN
    var darkIcon = false
        set(value) {
            field = value
            updateEnabledColor()
    ...
    private fun updateEnabledColor() {
        enabledColor = if (darkIcon) Color.WHITE else Color.BLACK
    }
    ...
    private fun animateBottomIcon(itemId: Int): Boolean {
        if (itemId != currentNavigationItemId) {
            ...
            if (currentNavigationItemId != -1) {
                ...
                currentView.drawable.setTint(Color.WHITE)
            ...
            animatorSet.playTogether(
                ...
                buildScaleIconAnimator(icon, 1.0f, 1.75f),
                buildScaleXIconAnimator(icon, 1.0f, 1.75f),
    ...
    private fun buildScaleIconAnimator(currentView: View, from: Float, to: Float):
            ObjectAnimator {
        return ObjectAnimator.ofFloat(
            currentView,
            "scaleY",
    ...
    private fun buildScaleXIconAnimator(currentView: View, from: Float, to: Float):
            ObjectAnimator {
        return ObjectAnimator.ofFloat(
            currentView,
            "scaleX",
